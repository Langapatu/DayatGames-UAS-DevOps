            ...
            <div class="space-y-4">
                @foreach($cart->items as $item)
                    <article class="cart-item-card">
                        <img src="{{ $item->game->coverUrl() }}" alt="Cover {{ $item->game->title }}" class="game-image-contain cart-item-cover">
                        ...
                        <form method="POST" action="{{ route('cart.destroy', $item->game) }}" class="cart-item-remove">
                            @csrf
                            @method('DELETE')
